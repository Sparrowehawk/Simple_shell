...
int aa_load_history(void){
    ...
    save_file = fopen(".hist_lists", "r");
    if(save_file == NULL){
        printf("aa: No file found, starting with new history\n");
        return 1;
    }

    while ((read = getline(&individual_line, &len, save_file)) != -1){ //something wrong here
        printf("%s\n", individual_line);
        char* command = calloc(read + 1, sizeof(char));
        
        if(!command){
            printf("aa: Error with memory\n");
            return 1;
        }

        strncpy(command, individual_line, read);
...
int aa_save_history(void){
    ...
    while(history[history_length]){
        history_length++;
    }

    save_file = fopen(".hist_lists", "w");

    if(save_file == NULL){
        printf("aa: Error saving to file, will save nothing\n");
        return 1;
    }

    for(int i = 0; i < history_length; i++){
        printf("%d of %d: %s\n", i, history_length, history[i]);
        fprintf(save_file, "%s\n", history[i]);
    }
